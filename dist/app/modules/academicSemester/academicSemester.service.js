...
const getAllSemesters = async (filters, paginationOptions) => {
    ...
    const result = await academicSemester_model_1.AcademicSemester.find(whereConditions)
        .sort(sortConditions)
        .skip(skip)
        .limit(limit);
...
const updateSemester = async (id, payload) => {
    ...
    const result = await academicSemester_model_1.AcademicSemester.findOneAndUpdate({ _id: id }, payload, {
        new: true,
    });
    return result;
};
const deleteSemester = async (id) => {
    const result = await academicSemester_model_1.AcademicSemester.findByIdAndDelete(id);
    return result;
};
