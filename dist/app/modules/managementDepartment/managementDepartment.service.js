...
const createManagementDepartment = async (data) => {
    ...
    try {
        ...
        const newManagementDepartment = await managementDepartment_model_1.ManagementDepartment.create([data], {
            session,
        });
...
const getAllManagementDepartment = async (filters, paginationOptions) => {
    ...
    const result = await managementDepartment_model_1.ManagementDepartment.find(whereConditions)
        .sort(sortConditions)
        .skip(skip)
        .limit(limit);
    const total = await managementDepartment_model_1.ManagementDepartment.countDocuments();
    return {
        meta: {
            page,
            limit,
...
const getSingleManagementDepartment = async (id) => {
    const result = await managementDepartment_model_1.ManagementDepartment.findOne({ _id: id });
    return result;
};
const updateManagementDepartment = async (id, payload) => {
    const result = await managementDepartment_model_1.ManagementDepartment.findOneAndUpdate({ _id: id }, payload, { new: true });
    return result;
};
